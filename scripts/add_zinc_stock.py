"""
Add Healthy Me Zinc stock item to Azure PostgreSQL database
- 143 units with barcodes ZN000001 to ZN000143
...
def add_zinc_stock():
    """Add Healthy Me Zinc stock item to Azure PostgreSQL."""
    with app.app_context():
        try:
            ...
            count = 143
            batch_number = "25G007"
            expiry_date = datetime(2027, 6, 1).date()  # June 2027
            provider = "Healthy Me"
            ...
            print(f"[OK] Expiry Date: {expiry_date.strftime('%B %Y')}")
            ...
            for i in range(start_num, start_num + count):
                barcode = f"{prefix}{i:06d}"  # ZN000001 format
            ...
            print(f"  Expiry Date: {expiry_date.strftime('%B %Y')}")
            print(f"  Total Stock Count: {stock_item.current_stock}")
            
            print("\n" + "="*60)
            print("[SUCCESS] Zinc Stock Added Successfully!")
            print("="*60)
